...
Option trading volumes are often much lower compared to stocks. This means that the spread between the bid and ask can be relatively wide making them more difficult to exit profitably.

# Terminology
### Profit & Loss
When you sell an option, you immediately gain the profit from the premium. However, the option still serves as a "liability" to you, since you may have to honor the option and sell (if CALL) or buy (if PUT) the stock if the strike price is reached.

When we sell an option
...
- to calculate how much of an option's premium is due to intrinsic value, an investor would subtract the strike price from the current stock price.
	- if the option premium is primarily made up intrinsic value, the option's value and profita
...
- Think of ownin a call option as putting up a down-payment for a future purchase. You could also see it as a deposit, in the sense that you can walk away from it and just be limited to losing your deposit.
...
### Covered Call
- sell a call option of a stock that you already own
...
#### Strategy with long-term stocks
By writing calls with high strike prices on stocks that you own, you can add modest amounts of return (~4% annually)
- strike price should be somewhere between 5-15% above the stock price. The strike price be a little above resistance, so as to decrease the odds that the strike price will be hit.
- call option positions can be closed by buying back options with the same strike price and same expiration date. It might make sense to do this if the stock reaches the strike price, in order to reduce your capital gains tax burden.
- As long as the option expires worthless (that is, the strike price never gets hit), then the P&L of the option itself doesn't really matter. All we care about here is the value of the premium. Therefore, a red P&L of the option value (indicating a rising stock price) is not a bad thing for us in this strategy, since we are already long on the stock. Our goal of course, is that the strike price never gets hit. Conversely, a green P&L of the option value (indicating a falling stock price) is fine, because we are owning the stock for the long-term anyway (ie. not phased by short term losses). We've already locked in our premium and the probability of the option being executed is low.
	- a positive P&L of the option is more of an indicator of the likelihood that the strike price will be hit and we'll have to sell the stock. The more in the green, the closer the stock price has risen toward the strike price. However, keep in mind that you still own the underlying stock, and are therefore bullish on it. You do in fact want the price to rise (which results in a more green P&L on the option's value), but you don't want it to rise too much so that the strike price gets hit. If it does however, we can just buy the call option back and purchase the shares again (in this case, the P&L can help to give an indication of the hit we will take if we want to buy back the option).
