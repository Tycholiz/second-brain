...
2. A filter envelope cuts out some of the harmonics (for example, by boosting or cutting all harmonics above a certain frequency).
3. An amplifier envelope changes the volume of the sound over time, according to ADSR values that you set (as we discussed above).
4. A second, independent oscillator, known as an LFO (low-frequency oscillator), can be used to vary how the previous three stages work, producing some very interesting effects. For example, if you apply the LFO to the original oscillator, it can make the frequency of the sound it generates wobble
...
- ex. If it is an amplitude envelope, then it shapes how the sound's amplitude changes over time.
...
When tweaking this knob, notice how we're modifying the sound in a similar way to how the sound is modified when we say "wah wah".

##### Resonance
Increases the amplitude around the cutoff frequency
- https://learningsynths.ableton.com/en/filters/filter-resonance
- that is, more resonance thins out the sound, because increasing resonance is increasing the volume of the top frequency of the filter
    - to hear this in action, increase the resonance, and move the filter around. You should hear the top frequency of the filter move around

[Anthony Marinelli](https://www.youtube.com/watch?v=22C6sATQyMg^)
