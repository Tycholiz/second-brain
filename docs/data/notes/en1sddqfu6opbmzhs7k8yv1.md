...
- As we go around the circle clockwise (starting from C), we notice that C has no sharps/flats, G has 1 sharp, D has 2 sharps, A has 3 etc. 
  ...
  - Also, D has the same sharps as G (plus one more), and A has the same sharps as D (plus one more), and so on
...
- if we start at the 4 chord and move clockwise, we'll notice that we start with the brightest mode, and gradually move towards darkness
  - modes in order of brightness-darkness is: lydian, ionian, mixolydian, dorian, aeolian, phygian, locrian
  - generally, the more sharps a mode has, the brighter it is. the more flats it has, the darker.
- A curious property of guide tones (ie. 3rd and dom7th) is that, given the guide tones of a dominant seventh chord, if you move both of them down a half-step you are now playing the guide tones of the next chord in the cycle. For example, in the cycle C moves to F. The guide tones of C7 are E and Bb. Moving those down a half-step, we get Eb and A—the guide tones of F7.
